(function() {
  ...
  function PassCtrl($scope, baConfig, $element, layoutPaths, passsServ,$timeout,$uibModal) {
    $scope.showPrintPass = true;
    $scope.name = {name : ""}
    $scope.adharNumbar = {adharNumbar : ""}
    ...
    $scope.windowsPrint = function(){
      $scope.showPrintPass = false; 
      setTimeout(function(){
        $scope.printPass()
    ...
    $scope.stateSelectOptions = $scope.state.map(function(item){
        var option  = { 
            label : item,
            value : item
        }
...
$scope.$watch('customDate', function (value) {
    ...
    $scope.selectedCity.selectedCityObj = {}
    $scope.cityNameBackup = $scope.cityName;
    $scope.cityName = "";
    $scope.getCitiesLimit();
    setTimeout(function(){
        $scope.cityChange($scope.cityNameBackup)
    }, 3000)
  ...
  $scope.$watch('selectedState.selectedStateObj', function (value) {
  ...
  $scope.cityChange = function(value){
    ...
    if($scope.cities && $scope.cities.length && $scope.cityName){
      $scope.filteredCityInfo = $scope.cities.filter(function(item){
          return item.cityName == $scope.cityName.label
        })
        if($scope.filteredCityInfo.length > 0 && $scope.filteredCityInfo[0].dailyLimit > 0){
            $scope.countryDailyLimit = true;
            $scope.dailyLimit = $scope.filteredCityInfo[0].dailyLimit;
        }
        else{
