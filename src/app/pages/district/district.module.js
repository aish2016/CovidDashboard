...
function DistrictCtrl($scope, baConfig, $element, layoutPaths, districtsService,$timeout) {
  $scope.selectedState = { selectedStateObj : []};
 $scope.getStatedata= function(){
          districtsService.getStatedata().then(
            ...
            $scope.stateSelectOptions = $scope.statelabeLunchUncked.map(function(item){
              var option  = { 
                  label : item,
                  value : item
              }
            ...
            $scope.selectedState.selectedStateObj = $scope.stateSelectOptions[1]
...
$scope.checgeState = function(selectedState){
   $scope.selectedState.selectedStateObj = selectedState
 $scope.setDistrictsData()
}

$scope.setDistrictsData = function(){
            $scope.districtlabeLunchUncked = Object.keys($scope.statedata[$scope.selectedState.selectedStateObj.label].districtData)
            $scope.districtDataCount = Object.keys($scope.statedata[$scope.selectedState.selectedStateObj.label].districtData).reduce(function(accumalator , district){
              accumalator[0].push($scope.statedata[$scope.selectedState.selectedStateObj.label].districtData[district].confirmed)
              accumalator[1].push($scope.statedata[$scope.selectedState.selectedStateObj.label].districtData[district].recovered)
              accumalator[2].push($scope.statedata[$scope.selectedState.selectedStateObj.label].districtData[district].deceased)
              return accumalator
            },[[],[],[]])
...
$scope.stackedBarData= []
            if($scope.statesDataConfirmedCounts.length> 0)
            $scope.stackedBarData.push({
              labels:  $scope.statesLabel[0],
              series: [$scope.statesDataDeathCounts[0], $scope.statesDataRecoveredCounts[0], $scope.statesDataConfirmedCounts[0]]
            })
                 if($scope.statesDataConfirmedCounts.length> 1)
                       $scope.stackedBarData.push({
              labels:  $scope.statesLabel[1],
              series: [$scope.statesDataDeathCounts[1], $scope.statesDataRecoveredCounts[1], $scope.statesDataConfirmedCounts[1]]
            })
                 if($scope.statesDataConfirmedCounts.length>2)
                       $scope.stackedBarData.push({
              labels:  $scope.statesLabel[2],
              series: [$scope.statesDataDeathCounts[2], $scope.statesDataRecoveredCounts[2], $scope.statesDataConfirmedCounts[2]]
            })
                  if($scope.statesDataConfirmedCounts.length> 3)
                       $scope.stackedBarData.push({
              labels:  $scope.statesLabel[4],
              series: [$scope.statesDataDeathCounts[4], $scope.statesDataRecoveredCounts[4], $scope.statesDataConfirmedCounts[4]]
            })
                  if($scope.statesDataConfirmedCounts.length>4)
                       $scope.stackedBarData.push({
              labels:  $scope.statesLabel[5],
              series: [$scope.statesDataDeathCounts[5], $scope.statesDataRecoveredCounts[5], $scope.statesDataConfirmedCounts[5]]
            })
                 if($scope.statesDataConfirmedCounts.length > 5)
                       $scope.stackedBarData.push({
              labels:  $scope.statesLabel[6],
              series: [$scope.statesDataDeathCounts[6], $scope.statesDataRecoveredCounts[6], $scope.statesDataConfirmedCounts[6]]
            })
